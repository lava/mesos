...
private:
  ...
  // NOTE: Most functions here are not directly routed but dispatched from
  // their corresponding because often deciding whether an incomding request
  // is read-only requires some inspection of the request, e.g. distinguishing
  // between "GET" and "POST" requests to the same endpoint.
  class ReadOnlyHandler
  {
  public:
    explicit ReadOnlyHandler(const Master* _master)
      : master(_master)
    {}
